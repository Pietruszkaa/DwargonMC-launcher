...
export async function readProfile(paths: LauncherPaths): Promise<LauncherProfile> {
  ...
  const legacyMicrosoft = profile.microsoft;
  const microsoft = normalizeMicrosoftProfile(legacyMicrosoft);
  const accountMode = profile.accountMode === 'microsoft' && microsoft ? 'microsoft' : 'offline';
  const hasLegacyToken = hasLegacyRefreshToken(legacyMicrosoft);

  if (hasLegacyToken && accountMode === 'microsoft' && microsoft) {
    try {
      await saveMicrosoftRefreshToken(microsoft.uuid, String(legacyMicrosoft.refreshToken));
    } catch {
