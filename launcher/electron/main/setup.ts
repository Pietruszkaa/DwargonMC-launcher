...
const INSTANCE_FOLDER_NAME = 'Dwargon Launcher';
const MANAGED_ENTRY_NAMES = new Set(['assets', 'launcher-data', 'minecraft', 'instances', INSTANCE_FOLDER_NAME]);
...
export async function resolveSetupPaths(
  ...
  const nestedPaths = buildLauncherPaths(nestedDir, basePaths.appDir);
  if (await pathExists(nestedPaths.profileFile)) {
    return setupResolution(nestedPaths, 'crowded-folder', true, []);
  }
  ...
  return setupResolution(nestedPaths, 'crowded-folder', true, crowdedEntries);
}
