...
describe('runSync', () => {
  it('downloads changed files and only removes orphan managed files', async () => {
    ...
    const paths: LauncherPaths = {
      installDir: temp,
      appDir: temp,
      globalDataDir: path.join(temp, 'launcher-data'),
      serversFile: path.join(temp, 'launcher-data', 'servers.json'),
      instancesDir: path.join(temp, 'instances'),
      activeInstanceId: 'dwargonmc',
      activeInstanceDir: temp,
